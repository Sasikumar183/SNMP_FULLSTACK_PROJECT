...
//        System.out.println(jsonArray.toString(4));
        List<JSONObject> sortedList = jsonArray.toList().stream()
            ...
            .sorted(Comparator.comparing(obj -> 
                obj.has("time_slot")|| obj.has("hour_slot")
                    ? LocalDateTime.parse(obj.has("time_slot")?obj.getString("time_slot"):obj.getString("hour_slot"), dateTimeFormatter) 
                    : LocalDate.parse(obj.optString("date") , dateFormatter).atStartOfDay(),
                Comparator.reverseOrder())) // Sort newest first
